...
# any idea how to get rid of 0084, and get 1984?
...
#5  
JulyLakes.PLOT <- ggplot(JulyLakes, aes(x = temperature_C, y = depth, color = depth))+
  geom_point()+ 
...
# p- values show the sigificance of the regression, we want a small p-value! 
...
> Answer: For every 1 (m) increase in depth, temperature decreases by 1.95 degree Celsius.The r-squared shows a 74% of the variability in changes in temperature is explained by changes in depth. 
The degrees of freedom are 9726.
The statistical significance shows shows a very low p-value.
the depth changes first than the temp changes, it is dependent.
---
...
# you want a higher AIC, so always remove variables that are below your start AIC. you want to repeat the removal process until you see (none). thats why its called a step algorithm
# everytime you add a explanatory variable you lose a degree of freedom. 
