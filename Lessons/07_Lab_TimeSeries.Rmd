...
# for statistics line... 
# the first Null represents the space before the Apr in: " Apr Average"
# while Null2 represents "Average"
# and the " " after shows that you are separating for columns 
...
print(RainfallPlot)  
 ...
 #  NO MISSING DATA FOR EITHER PLOT
...
library(cowplot)
DoubleTheFun <- plot_grid(TempPlot,RainfallPlot, nrow = 1)
print(DoubleTheFun)

# temperature increases, while rainfall remains stagnate over time. 
```
...
f_month_T <- month(first(Temp_BR_processed$Date))
f_year_T <- year(first(Temp_BR_processed$Date))
Temp_ts <- ts(Temp_BR_processed$Temperature_C,
                   start=c(f_year_T,f_month_T),
                   frequency=12)   

f_month_R <- month(first(Rainfall_BR_processed$Date))
f_year_R <- year(first(Rainfall_BR_processed$Date))
Rainfall_ts <- ts(Rainfall_BR_processed$Rainfall,
                   start=c(f_year_R,f_month_R),
                   frequency=12)  
...
Rain_decomp <- stl(Rainfall_ts, s.window = "periodic") 
plot(Rain_decomp)
#first is seasonal data, 
#second is seasonal component 
#third is trend 
#fourth is remainder of your data 
...
# Shirley look at the picture you took in class 
Extracted_temp <- 
